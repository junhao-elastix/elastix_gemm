// ------------------------------------------------------------------
// Testbench for comp_mlp_bram_col_wrapper.sv
//
...
module tb_mlp_wrapper;
    ...
    always_ff @(posedge clk) begin
        if (dout_valid && dout_ready) begin
            // Wrapper outputs 8 MLPs, each with bank0 (even) and bank1 (odd) results
            // o_dout[m] = {40'd0, final_bank1[m], final_bank0[m]}
            for (int m = 0; m < NUM_MLPS; m++) begin
                // Even column = bank0 = bits [15:0]
                collected_results[num_results_collected + m*2]     = dout[m][15:0];
                // Odd column = bank1 = bits [31:16]
                collected_results[num_results_collected + m*2 + 1] = dout[m][31:16];
            end
            num_results_collected = num_results_collected + NUM_COLUMNS;
            $display("[TB] @%0t Collected %0d results, MLP0: bank0=0x%04x bank1=0x%04x",
                     $time, num_results_collected, dout[0][15:0], dout[0][31:16]);
        end
    end
    ...
    initial begin
        ...
        $display("\n========================================");
        $display("comp_mlp_bram_col_wrapper Testbench");
