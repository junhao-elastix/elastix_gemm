...
module comp_mlp_bram #(
  ...
  // Debug: Track BRAM read data
  always @(posedge clk) begin
      if (ce && !wren) begin
          // Log BRAM output during compute (not during weight loading)
          $display("[BRAM_READ] @%0t rdaddr=%0d dout[71:0]=0x%018x dout[143:72]=0x%018x",
                   $time, rdaddr, bram_dout[71:0], bram_dout[143:72]);
      end
      // Debug: Track MLP input and output
      if (ce && |din) begin
          $display("[MLP_DIN] @%0t din=0x%018x load=%b", $time, din, load);
      end
  end
  `endif
