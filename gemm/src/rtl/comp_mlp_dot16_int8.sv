...
// Table 107: Sixteen Multipliers (×4 Mode – bytesel_00_07 = 'h01; bytesel_08_15 = 'h21)
    parameter logic [4:0]  bytesel_00_07                 = 5'h01,  // Lower bank: BFP Int8 ×2/×4 mode (8 multiplications)
                                                                   ...
                                                                   // 5'h1E -
                                                                   // 5'h1F - Int32 mode
    parameter logic [5:0]  bytesel_08_15                 = 6'h21,  // Upper bank: BFP Int8 ×2 mode (8 multiplications)
                                                                   // 6'h00 - Int8 ×1 mode
                                                                   // 6'h01 - Int8 ×2 mode
                                                                   // 6'h02 -
    ...
    parameter logic del_expa_reg                         = 2'h0,   // Number of delay stages applied to floating point A input sign and
                                                                   // exponent from byte selection to FP_MULT_AB.
                                                                   // 2 or 3 stages supported?
    parameter logic [1:0]  del_expb_reg                  = 2'h0,   // Number of delay stages applied to floating point B input sign and
                                                                   // exponent from byte selection to FP_MULT_AB.
    parameter logic [1:0]  del_expc_reg                  = 2'h0,   // Number of delay stages applied to floating point C input sign and
                                                                   // exponent from byte selection to FP_MULT_CD.
    parameter logic [1:0]  del_expd_reg                  = 2'h0,   // Number of delay stages applied to floating point D input sign and
                                                                   // exponent from byte selection to FP_MULT_CD.
    parameter logic [3:0]  cesel_expta_reg               = 4'h0,   // Selects the ce inputs for each delay stage register:
    parameter logic [3:0]  cesel_exptb_reg               = 4'h0,   //
    parameter logic [3:0]  cesel_exptc_reg               = 4'h0,   //
    parameter logic [3:0]  cesel_exptd_reg               = 4'h0,   //
    parameter logic [2:0]  rstsel_expta_reg              = 3'h0,   // Selects the rstn input for each delay stage register
    parameter logic [2:0]  rstsel_exptb_reg              = 3'h0,   //
    parameter logic [2:0]  rstsel_exptc_reg              = 3'h0,   //
    parameter logic [2:0]  rstsel_exptd_reg              = 3'h0,   //

    // FP Configuration

    parameter logic        fpmult_ab_blockfp             = 1'b0,   // Select (A×B) regular floating point or block floating point:
    ...
    parameter logic [2:0]  fpmult_ab_blockfp_mode        = 3'b000, // Select size of integer multipliers for (A*B) block floating point
                                                                   ...
                                                                   // 3'b110: 6*6
                                                                   // 3'b111: 7*7
    parameter logic        fpmult_cd_blockfp             = 1'b0,   // Select (C×D) regular floating point or block floating point:
                                                                   // 1'b0 – ReguIar floating point (input – floating-point numbers)
                                                                   // 1'b1 – Block floating point (input – integer mantissas and shared exponent)
    ...
    parameter logic        fpmult_ab_bypass              = 1'b1,   // multiplication mode selection
    ...
    parameter logic        fpmult_cd_bypass              = 1'b1,   // multiplication mode selection
    ...
    parameter logic        add_accum_ab_bypass           = 1'b0,   // bypass to the AB accumulator output
                                                                   // 1'b0 – integer AB accumulator value is used
                                                                   // 1'b1 – bypass integer AB accumulator or FPADD_AB accumulator
    parameter logic        accum_ab_reg_din_sel          = 1'b0,   // Select between integer (or bypass) and floating-point AB result
    ...
    parameter logic        add_accum_cd_bypass           = 1'b0,   // Select to bypass the CD accumulator output
                                                                   // 1'b0 – CD accumulator value is used.
                                                                   // 1'b1 – Bypass CD accumulator and use signal selected with  fpadd_cd_dina_sel


    parameter logic [2:0]  out_reg_din_sel               = 3'b011, // Select out_reg input - bypass floating-point value and accumulator value
    ...
    parameter logic [3:0]  cesel_fp_format_ab_reg        = 4'h0,   // select ce
    parameter logic [3:0]  cesel_fp_format_cd_reg        = 4'h0,   //
    parameter logic [2:0]  rstsel_fp_format_ab_reg       = 3'h0,   // select rstn
    parameter logic [2:0]  rstsel_fp_format_cd_reg       = 3'h0,   //
    ...
    parameter logic [1:0]  outmode_sel                   = 2'b00,  // Select final DOUT value:
