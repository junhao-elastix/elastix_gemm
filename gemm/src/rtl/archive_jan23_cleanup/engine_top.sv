...
(
    ...
    // DISPATCH done signal - tied high since compute_engine_mlp doesn't have DISPATCH interface
    // This allows master_control to proceed past DISPATCH wait states
    assign ce_mc_disp_done = 1'b1;
    ...
    // NOTE: DISPATCH ports removed - weight loading via vectorized i_wt_wr_* interface
    //       For now, weight interface tied to safe defaults (no weight loading)
    compute_engine_mlp #(
        .MATMUL_ID          (0),
        .NUM_MLPS           (NUM_TILES)
        ...
        // row_bram Write Interface (Left side - activations)
        ...
        // Weight Write Interface (vectorized - for RIGHT side loading)
        // NOTE: Currently disabled - tie to safe defaults
        .i_wt_wr_en          (1'b0),              // No weight writes for now
        .o_wt_wr_ready       (),                  // Unused
        .i_wt_wr_man         (256'd0),            // Safe default
        .i_wt_wr_exp         (8'd0),              // Safe default
        .i_wt_mlp_sel        (3'd0),              // Safe default
        .i_wt_nv_idx         (10'd0),             // Safe default
