...
(
    ...
    // Payload storage (up to 2 words for cmd_matmul_s)
    ...
    // Parsed TILE command structure (combinational decode)
    cmd_matmul_s tile_cmd;
    assign tile_cmd = {payload_word3_reg, payload_word2_reg, payload_word1_reg};
        ...
        case (state_reg)
            ...
            ST_DECODE: begin
                ...
                case (cmd_op_reg)
                    ...
                    e_cmd_op_matmul:      state_next = ST_EXEC_TILE;
                    e_cmd_op_wait_disp: state_next = ST_WAIT_DISP;
                    e_cmd_op_wait_matmul: state_next = ST_WAIT_TILE;
    ...
    always_ff @(posedge i_clk) begin
        if (~i_reset_n) begin
            ...
            if (state_reg == ST_EXEC_TILE) begin
                // Parse cmd_matmul_s structure (SPEC-COMPLIANT, 96-bit):
                ...
                // tile_cmd computed combinationally at module level
    ...
    always_ff @(posedge i_clk) begin
        if (~i_reset_n) begin
            ...
            if (state_reg == ST_DECODE && (cmd_op_reg == e_cmd_op_wait_disp || cmd_op_reg == e_cmd_op_wait_matmul)) begin
