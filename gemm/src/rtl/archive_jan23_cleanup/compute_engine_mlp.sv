...
//   - MATMUL: MATMUL computation (row_bram activations × mlp_bram weights → results)
//
...
module compute_engine_mlp #(
    parameter int MATMUL_ID = 0,                // Tile ID for debugging
    ...
    // =========================================================================
    // Master Control Interface (MATMUL command)
    // =========================================================================
    ...
    input  logic [15:0]              i_tile_right_addr,      // MATMUL: Right matrix read base address
    ...
    // Exponent conversion: E5 → E8 for MLP BFP mode
    // E5 bias = 15, E8 bias = 127
    // To preserve actual exponent: E8_val = E5_val + (127 - 15) = E5_val + 112
    always_comb begin
        for (int i = 0; i < 4; i++) begin
            nv_left_exp[i*8 +: 8] = nv_left_exp_raw[i*8 +: 8] + 8'd117;
        end
    end
    ...
    // Weight loading ready signal (wrapper is always ready)
    logic        wt_wr_ready;
    assign wt_wr_ready = 1'b1;  // Always ready - no backpressure from wrapper
    ...
    // Active parameters (from MATMUL or DISPATCH command)
    logic [7:0]  active_vec_len;        // V: from MATMUL or DISPATCH
    logic [7:0]  active_right_ugd_len;  // C: from MATMUL or DISPATCH
    logic [7:0]  active_left_ugd_len;   // B: from MATMUL
    logic [15:0] active_left_addr;      // Left base address for row_bram reads: from MATMUL
    logic [15:0] active_right_addr;     // Right base address for row_bram reads: from DISPATCH (fill) or MATMUL (compute)
    ...
    // =========================================================================
    // Weight Exponent Conversion: E5 → E8 for MLP BFP mode
    // E5 bias = 15, E8 bias = 127
    // To preserve actual exponent: E8_val = E5_val + (127 - 15) = E5_val + 112
    // =========================================================================
    logic [7:0] wt_line_exp_e8;
    assign wt_line_exp_e8 = i_wt_wr_exp + 8'd117;
    ...
    comp_mlp_bram_col_wrapper #(
        ...
        // Read base address configuration (for MATMUL compute)
        ...
        .i_new_dot(new_dot),
        .i_last_nv(last_nv),
        .i_last_matmul(last_matmul),  // Truly last dot product of entire MATMUL
