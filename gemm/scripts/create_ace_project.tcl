...
# Optional argument, may not always be present
# Support 2 options for two-stage bitstream generation, no and yes
if { [llength $argv] > 9 } {
    set generate_two_stage_bitstream [string tolower [lindex $argv 9]]
} else {
    set generate_two_stage_bitstream "no"
}
...
# Firstly check the ACE version
# If it's 10.0 or greater, then can add the synthesis files and constraints.
set ace_major_version [lindex [split [get_ace_version] "."] 0]
puts "ACE major version is $ace_major_version"
...
if { [info exists ace_constraints_files] } {
    foreach constraint [strip_comments $ace_constraints_files] {
        # Allow for case where file path uses variables
        set constraint [subst $constraint]
...
# Set a flag to indicate if ACE is going to do the synthesis
# This is enabled if ACE version is 10.0 or greater, and at least one RTL file was found for synthesis
set ace_synthesis 0

# For ACE 10.0 and above, we can add synthesis files
# Must always call this block for ACE 10.0 and onwards as besides determining whether to use integrated
# synthesis, it also turns off unwanted flow steps
if { ([expr $ace_major_version] >= 10)  || ([expr $ace_major_version] == 0)} {

    # By default, disable synthesis.
    # If any of the blocks below have any files, then enable the synthesis step
    disable_flow_step "run_synthesis"

    if { [info exists rtl_verilog_files] } {
        foreach fname [strip_comments $rtl_verilog_files] {
            add_project_source_files -syn_rtl -project $projectName [file join $path_to_filelist "rtl" $fname]
            set ace_synthesis 1
        }
    }
    ...
    if { [info exists synplify_constraints_files] } {
        foreach fname [strip_comments $synplify_constraints_files] {
            add_project_source_files -syn_constraint -project $projectName [file join $path_to_constraints $fname]
        }
    }

    # If the rev is not set to "ace", then synthesis will be done separately by Synplify Pro
    # This overrides the enabling of ace_synthesis above
    if { $rev != "ace" } { set ace_synthesis 0 }

    if { $ace_synthesis } {
        enable_flow_step "run_synthesis"

        # Cannot do as relative path, ACE then does it relative to it's generated output synthesis path, (ace/impl/syn/ etc)
    ...
    # ------------------------------
    # As this script is used for synthesis and PnR, always turn of simulation in this generated ACE project
    disable_flow_step "run_simulation_rtl"

    # Turn off the auto-generation of all IP files.  This is controlled separately by the options below
    disable_flow_step "generate_all_ip_design_files"

}

# Include a netlist based on whether ACE is performing synthesis or not
if { $ace_synthesis == 0 } {
...
if { $generate_ioring != "no" } {
    ...
    if { ([expr $ace_major_version] >= 10) } {
        # ACE version 10.0 onwards
        if { $generate_ioring == "all" } {
            # Generate all IP files, not just ioring IP files.
            puts "----------------------------------------------------------------------"
            puts "Generate all IP output files; Generating files in $generate_ioring_path"
            puts "----------------------------------------------------------------------"
            regenerate_all_ip_design_files -ioringOutputDir $generate_ioring_path -add_to_project
        } else {
            # Generate just IOring IP files
            puts "----------------------------------------------------------------------"
...
if { ($generate_ioring == "only") || ($generate_ioring == "all") } {
    puts "-----------------------------------------------------------------------------"
...
if { ($mp == "-multiprocess") || ($mp != 0) } {
    ...
    # Flag will only be set for ACE 10.0 and higher
    if { $ace_synthesis }  {
        run -step "run_synthesis"
    }

    # Multiprocess run
    if { ($jobs > 1) && ($jobs < 10) } {
        if { ([expr $ace_major_version] >= 10) } {
            # New flow for ACE 10.0 onwards
            run_multiprocess -parallel_job_count $jobs -create_option_sets -stop_flow report_timing_final
        } else {
            # Original flow for ACE prior to 10.0
            ...
            run_multiprocess -parallel_job_count $jobs -stop_flow report_timing_final
        }
    } else {
    ...
    # Standard run
    run
    if { $generate_two_stage_bitstream != "no" } {
        set_impl_option -project $projectName -impl $implName  "bitstream_output_pcie"  "1"
        set_impl_option -project $projectName -impl $implName  "bitstream_two_stage"  "1"
        save_project -no_db
