...
// ============================================================================
#define ENGINE_CMD_WORD0        0x28    // Command word 0 (opcode)
#define ENGINE_CMD_WORD1        0x2C    // Command word 1
#define ENGINE_CMD_WORD2        0x30    // Command word 2 (FIXED: was 0x2D, should be 0x30)
#define ENGINE_CMD_WORD3        0x34    // Command word 3
#define ENGINE_CMD_SUBMIT       0x38    // Submit trigger (write 1)
#define ENGINE_STATUS           0x3C    // {CE[3:0], DC[3:0], MC[3:0], busy}
#define ENGINE_RESULT_COUNT     0x40    // FP16 result count
...
#define VLOOP_SIZE              1       // V parameter (vectors per row/col) - TESTING V=1 for debug
...
#define BRAM_RESULT_BASE        0x4460008000ULL // BRAM responder base address
...
int main(int argc, char* argv[]) {
    ...
    try {
        ...
        uint32_t bitstream_id = device.mmioRead32(0, 0x1F8);
        ...
        uint32_t adm_status = device.mmioRead32(0, 0x1F4);
        ...
        uint32_t debug_reg = device.mmioRead32(0, 0x44);  // ENGINE_DEBUG register
        uint32_t status_reg = device.mmioRead32(0, 0x3C);  // ENGINE_STATUS register
        ...
        cout << "  ENGINE_STATUS (before wait): 0x" << hex << status_reg
             << " (mc_state_next=" << ((status_reg >> 12) & 0xF)
             << ", mc_state=" << ((status_reg >> 8) & 0xF)
             << ", dc_state=" << ((status_reg >> 4) & 0xF)
             << ", ce_state=" << (status_reg & 0xF) << ")" << dec << endl;
        ...
        if (compareResults(result_float, golden_result, 0.05f)) {  // 5% tolerance for FP16
