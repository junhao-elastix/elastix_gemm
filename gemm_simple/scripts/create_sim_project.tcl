...
proc write_filelist_header { fo fptr infile_name {vhdl 0} } {
    # Add banner
    if { $vhdl } {
        puts $fo "\n// -----------------------------------------------"
...
proc write_filelist_tail { fo fptr infile_name {vhdl 0} } {
    # Write tail
    if { $vhdl } {
        puts $fo "\n// -------------------------------------"
...
if { ![file exists $input_filelist] } {
    puts "No such file $input_filelist.  Current dir [pwd]"
    return -1
} else {
...
write_filelist_header $fpo $fp_template $input_filelist
...
# Can also require ioring directory.  Add if present. Use override if available
if { [info exists generate_ioring_path] } {
    # The $generate_ioring_path is relative to the src/ace directory, (see create_ace_project)
    set path_to_ioring [file join $path_to_filelist/ace $generate_ioring_path]
} else {
    set path_to_ioring [file join $path_to_filelist "ioring"]
}
# Only add ioring path if it already exists
if { [file exists $path_to_ioring] } {
    puts $fpo "# IOring files include directory"
    puts $fpo "+incdir+$path_to_ioring"
}
...
# ---------------------------------------------------------------------
if { [info exists input_template_vhdl] && \
     ( ([info exists rtl_vhdl_files] && [llength $rtl_vhdl_files]) || \
       ([info exists tb_vhdl_files]  && [llength $tb_vhdl_files]) ) } {
    set vhdl_required 1
} else {
    set vhdl_required 0
}
...
if { $vhdl_required } {
    ...
    if { $fpv == 0} {
        puts "Cannot create output file $output_filename_vhdl"
        return -1
    }

    # Write the filelist header
    # VHDL requires different comment lines
    write_filelist_header $fpv $fp_template_vhdl $input_filelist 1
    ...
    if { [info exists tb_vhdl_files] } {
        puts $fpv "\n// VHDL testbench files"
        foreach fname [strip_comments $tb_vhdl_files] {
            puts $fpv "[file join $path_to_tb $fname]"
        }
    }

    # Write end of filelist
    write_filelist_tail $fpv $fp_template_vhdl $input_filelist 1
