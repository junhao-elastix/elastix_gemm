...
#
# NOTE: This uses the HARDWARE-ACCURATE algorithm via hardware_gfp_reference_new.py

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
HEX_DIR="$SCRIPT_DIR"
REF_SCRIPT="$SCRIPT_DIR/../hardware_gfp_reference_new.py"

# Check if reference script exists
if [ ! -f "$REF_SCRIPT" ]; then
    echo "ERROR: hardware_gfp_reference_new.py not found at $REF_SCRIPT"
    exit 1
fi
...
echo "Using HARDWARE-ACCURATE algorithm (hardware_gfp_reference_new.py)"
echo "=============================================="
echo ""
