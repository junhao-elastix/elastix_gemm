...
NOTE: This module uses the EMULATOR-BASED GFP GEMM algorithm for comparison.
For hardware-accurate golden reference generation, use hardware_gfp_reference.py.
...
from hex_utils import (
    ...
    decode_exponents,
    decode_mantissas,
    convert_gfp_to_float,
    setup_emulator_import,
    create_bcv_parser,
    validate_bcv_constraints,
)

# Setup emulator import path
setup_emulator_import()
...
class GFPMatrix:
    ...
    def _load_and_decode(self):
        ...
        # Load raw hex data using canonical utility
        exp_raw, man_raw = load_hex_file(self.filename)

        # Decode exponents and mantissas using canonical utilities
        ...
        # Convert to float matrix using canonical utility
        self.float_matrix = convert_gfp_to_float(self.mantissa_data, self.exp_data)
...
def test_gemm_decoding(B: int = 1, C: int = 1, V: int = 1):
    ...
    NOTE: This uses the EMULATOR-BASED GFP GEMM algorithm, which may differ slightly
    from the hardware-accurate algorithm. For hardware golden reference, use
    hardware_gfp_reference.py instead.
    ...
    try:
        print("=" * 80)
        print(f"GFP Matrix Decoding and EMULATOR-BASED GEMM Test (B={B}, C={C}, V={V})")
        print("NOTE: This uses EMULATOR algorithm. For hardware-accurate results,")
        print("      use hardware_gfp_reference.py instead.")
        print("=" * 80)
        ...
        print("\n3. Creating GFP tensors (EMULATOR-BASED)...")
        dtype = GFPDataType(
        ...
        print("\n4. Setting up EMULATOR GFP GEMM operation...")
        ...
        # Step 5: Perform GEMM
        print("\n5. Performing EMULATOR GFP GEMM...")
        gfp_result = gemm(gfp_a, gfp_b)
        result = gfp_result.dequantize().numpy()

        # Step 6: Display results
        print(f"\n6. EMULATOR GEMM Result ({result.shape[0]}x{result.shape[1]}) - Expected: {B}x{C}:")
        print("=" * 80)
        ...
        print(f"\n7. Comparison: FP Matmul vs EMULATOR GFP GEMM Results")
        print("=" * 80)
        ...
        if not close_enough:
            print(f"   Warning: Significant differences detected between FP and EMULATOR GFP results!")
            print(f"   This may indicate precision loss in GFP quantization or computation differences.")
        else:
            print(f"   Results are consistent between FP Matmul and EMULATOR GFP GEMM.")

        # Step 8: Save results to file
        output_path = "decoded_result.txt"
        with open(output_path, 'w') as f:
            f.write("EMULATOR GFP GEMM vs FP Matmul Comparison - Decoded from Hex Files\n")
            f.write("NOTE: This uses EMULATOR algorithm. For hardware-accurate results,\n")
            f.write("      use hardware_gfp_reference.py instead.\n")
            f.write("=" * 80 + "\n")
            ...
            f.write(f"EMULATOR GFP GEMM Statistics: Min={np.min(result):.6f}, Max={np.max(result):.6f}, Mean={np.mean(result):.6f}\n")
            ...
            f.write("\n" + "=" * 80 + "\n")
            f.write("EMULATOR GFP GEMM Result Matrix (Float values):\n")
            f.write("=" * 80 + "\n")
            ...
            f.write("\n" + "=" * 80 + "\n")
            f.write("Difference Matrix (FP - EMULATOR GFP):\n")
            f.write("=" * 80 + "\n")
            ...
            f.write("\n" + "=" * 80 + "\n")
            f.write("EMULATOR GFP GEMM Result Matrix (Hex values - IEEE 754 float16):\n")
            f.write("=" * 80 + "\n")
            for r in range(result.shape[0]):
                for c in range(result.shape[1]):
                    float16_val = np.float16(result[r, c])
        ...
        print("\n" + "=" * 80)
        print("EMULATOR GFP GEMM vs FP Matmul Comparison Test Completed Successfully!")
        print("=" * 80)
...
if __name__ == "__main__":
    ...
    Usage:
        python mem_layout_new.py [--B B] [--C C] [--V V]

    Default: B=C=V=1 (128x128 matrices)
    """
    # Parse command line arguments using canonical utility
    parser = create_bcv_parser("GFP Matrix Decoder with configurable dimensions (EMULATOR-BASED)")
    args = parser.parse_args()

    # Validate constraints using canonical utility
    try:
        validate_bcv_constraints(args.B, args.C, args.V)
    except ValueError as e:
        print(f"Error: {e}")
        sys.exit(1)

    print(f"Running EMULATOR GFP GEMM with parameters: B={args.B}, C={args.C}, V={args.V}")
    ...
    if result is not None:
        ...
        print("\n[FAIL] Test failed - see error messages above")
